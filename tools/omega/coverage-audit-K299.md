...
corpus `src/components/omega-corpus-mrgrey.json` · engine `src/components/yurei-oracle.js` · entries 185 · patterns 850 · fresh Matcher per probe · measure-only

## Findings (numbers; the reading is in the K295 stratum)

- Crisis floor: fires by construction at stage 1 (any hit, score>0, no threshold, before every other lane); bare 29/29, embedded 140/140, repeated ×4 and post-conversation all crisis. The floor's own coverage of adjacent distress phrasings: 9/24 (§2).
- Self-match (control): 99.18% — every failure is a declared collision or the floor arbitrating with itself; undeclared steals: 0 (§3).
- Ambiguity: 6 declared collisions; 1 true coin flips (score-tied, decided by alphabetical id: `hi`); 12 score ties decided by matched length, i.e. the specificity rule working (§4).
...
- Brittleness: `contains` forms survive a leading filler 99.9% and a trailing one 99.9%; `exact` forms survive a leading filler 1.1%; contraction expansion survives 33.3%; word order 4.1%; one-edit typos ≈1% (§6).
- Subject coverage: 82 library objections, 53 signed; 9 signed objections that no library phrasing or keyword reaches; 34 signed objections with deflecting title phrasings (candidate triggers); 6 cross-position landings (§7).
- Help: 18/32 help-shaped inputs deflect (§8).
- Dynamics: a rephrase of the same response entry within the 8-turn dampening window deflects for 149/178 entries (§9).
...
- bare forms -> crisis: 29/29 · embedded/stacked -> crisis: 140/140 · repeated x4 on one matcher: crisis,crisis,crisis,crisis · after an oracle + a position hit: crisis
...
**The floor's own coverage — adjacent phrasings NOT declared (report-only; the floor is a ratified surface shared with Yūrei):** 9/24 reach crisis.
...
## 3 Self-match (control) — 99.18% (843/850)
...
Score-tied forms (distinct): 13 — 12 decided by matched length (the specificity rule: the longer form wins over the shorter one it contains; deterministic, and the intended behaviour), 1 decided by alphabetical id (true coin flips). Winning by a margin of 2 or less: 20.
...
`MISS_THRESHOLD` = 16 is inert: the smallest possible match score is 24 (responses 61). Scores are a MAX over matching patterns, never a sum — the kickoff's "threshold headroom" axis cannot exist. Entries with patterns 178 · exact-only 5 · exactly one self-retrieving form 0 · zero 0.
...
|---|---|--:|--:|--:|--:|--:|
| contract | contains | 75 | 92 | 6 | 0 | 0 |
| contract | exact | 5 | 80 | 1 | 0 | 0 |
| ctl-apostrophe | contains | 12 | 100 | 0 | 0 | 0 |
| ctl-punct | contains | 728 | 100 | 0 | 0 | 0 |
| ctl-punct | exact | 93 | 100 | 0 | 0 | 0 |
| ctl-space | contains | 728 | 100 | 0 | 0 | 0 |
| ctl-space | exact | 93 | 100 | 0 | 0 | 0 |
| ctl-upper | contains | 728 | 100 | 0 | 0 | 0 |
| ctl-upper | exact | 93 | 100 | 0 | 0 | 0 |
| expand | contains | 12 | 33.3 | 8 | 0 | 0 |
| hyphen | contains | 641 | 100 | 0 | 0 | 0 |
| hyphen | exact | 16 | 100 | 0 | 0 | 0 |
| lead | contains | 5824 | 99.9 | 0 | 5 | 0 |
| lead | exact | 744 | 1.1 | 729 | 7 | 0 |
| order | contains | 640 | 4.1 | 605 | 9 | 0 |
| order | exact | 16 | 0 | 16 | 0 | 0 |
| plural | contains | 727 | 1.7 | 699 | 16 | 0 |
| plural | exact | 93 | 0 | 93 | 0 | 0 |
| trail | contains | 2912 | 99.9 | 0 | 2 | 0 |
| trail | exact | 372 | 1.1 | 367 | 1 | 0 |
| typo-del | contains | 718 | 1 | 703 | 8 | 0 |
| typo-del | exact | 67 | 0 | 67 | 0 | 0 |
| typo-dup | contains | 718 | 1 | 703 | 8 | 0 |
| typo-dup | exact | 67 | 0 | 67 | 0 | 0 |
| typo-swap | contains | 718 | 4.6 | 677 | 8 | 0 |
| typo-swap | exact | 67 | 10.4 | 60 | 0 | 0 |

Wrong-answer mutants (a mutated form landing on a DIFFERENT non-deflection entry): 64, of which 41 are NESTED FALLBACKS — the broken form still contains a shorter form of the entry it fell to. The catch-alls:
...
| `mg-stance-bait-02` | 2 | 1 | your philosophy x1, are you a nihilist x1 |
...
| `pos-nihilism-label-01` | `you are a nihilist` | order | `are you a nihilist` | `mg-stance-bait-02` | `are you a nihilist` | no |
...
| heading | 206 | 186 | 17 | 2 | 0 | 0 | 1 |
| library-title | 274 | 151 | 19 | 3 | 96 | 2 | 3 |
| page | 44 | 29 | 14 | 0 | 0 | 0 | 1 |
| void | 25 | 24 | 1 | 0 | 0 | 0 | 0 |
| library-keyword | 696 | 646 | 7 | 10 | 27 | 3 | 3 |
...
| `consent-both-ways` | yes | 1 | 4 | 2 | 0 | 0 | 0 | 3 |
| `consent-incoherent` | yes | 3 | 5 | 1 | 0 | 1 | 0 | 6 |
...
| `non-identity-problem` | yes | 3 | 8 | 2 | 0 | 0 | 0 | 9 |
...
| `violence-as-reductio` | yes | 6 | 15 | 0 | 0 | 0 | 1 | 20 |
...
| `consent-both-ways` | `the unborn can not consent to non existence either` | — |
| `consent-incoherent` | `you can not get consent from non existent beings` | `the consent argument is incoherent->mg-oracle-argue-01` |
...
| `non-identity-problem` | `the non identity problem` · `you can not harm someone who would not otherwise exist` | — |
...
| glossary | `modal architectural pessimism` |
...
| library-title | `the unborn can not consent to non existence either` (consent-both-ways) |
| library-title | `you can not get consent from non existent beings` (consent-incoherent) |
...
| library-title | `suffering is just c fibers firing` (eliminativism, unsigned) |
| library-title | `badness is a folk illusion` (eliminativism, unsigned) |
| library-title | `qualia do not exist` (eliminativism, unsigned) |
| library-title | `phenomenal suffering is a user illusion` (eliminativism, unsigned) |
...
| library-title | `you can not have pleasure without suffering` (hedonic-contrast, unsigned) |
...
| library-title | `you can not harm someone who would not otherwise exist` (non-identity-problem) |
...
| library-title | `how do you know there is anyone there to harm` (solipsism, unsigned) |
...
| library-title | `suffering focused ethics demands ecological terrorism` (wild-animal-suffering-consistency, unsigned) |
...
| aux-wing-id | `mentally ill can not consent` (mentally-ill-cant-consent) |
...
| heading | _… 146 more in the JSON_ |
| library-keyword | _… 606 more in the JSON_ |
...
- Rephrase within the 8-turn dampening window (form A then form B of the same entry, one matcher): 178 entries → same entry 24 · deflection 149 · a different entry 5.
