...
corpus `src/components/omega-corpus-mrgrey.json` · engine `src/components/yurei-oracle.js` · entries 184 · patterns 833 · fresh Matcher per probe · measure-only

## Findings (numbers; the reading is in the K295 stratum)

- Crisis floor: fires by construction at stage 1 (any hit, score>0, no threshold, before every other lane); bare 18/18, embedded 85/85, repeated ×4 and post-conversation all crisis. The floor's own coverage of adjacent distress phrasings: 2/24 (§2).
- Self-match (control): 99.16% — every failure is a declared collision or the floor arbitrating with itself; undeclared steals: 0 (§3).
- Ambiguity: 6 declared collisions; 2 true coin flips (score-tied, decided by alphabetical id: `hi`, `edgelord`); 12 score ties decided by matched length, i.e. the specificity rule working (§4).
- Reachability: `MISS_THRESHOLD` is inert (min score 24); no entry is unreachable fresh; 5 entries are exact-only (§5).
- Hyphens: the normalization law deletes them with no substitution, so a hyphenated compound survives 2.8% of the time on `contains` forms (§6); typed apostrophes (`you're`, `don't`) are no-ops by the same law (control).
- Brittleness: `contains` forms survive a leading filler 99.9% and a trailing one 100%; `exact` forms survive a leading filler 1.1%; contraction expansion survives 14.9%; word order 3.9%; one-edit typos ≈1% (§6).
- Subject coverage: 82 library objections, 53 signed; 9 signed objections that no library phrasing or keyword reaches; 33 signed objections with deflecting title phrasings (candidate triggers); 6 cross-position landings (§7).
- Help: 27/32 help-shaped inputs deflect (§8).
- Dynamics: a rephrase of the same response entry within the 8-turn dampening window deflects for 147/177 entries (§9).
...
| `help` | `mg-oracle-nav-01` | `mg-oracle-nav-01` | oracle |
...
- bare forms -> crisis: 18/18 · embedded/stacked -> crisis: 85/85 · repeated x4 on one matcher: crisis,crisis,crisis,crisis · after an oracle + a position hit: crisis
...
**The floor's own coverage — adjacent phrasings NOT declared (report-only; the floor is a ratified surface shared with Yūrei):** 2/24 reach crisis.

| phrasing | lane | entry |
|---|---|---|
| `i want to end things` | deflection | `mg-deflect-01` |
| `i dont want to exist` | deflection | `mg-deflect-01` |
| `i want to disappear` | deflection | `mg-deflect-01` |
| `i cant do this anymore` | deflection | `mg-deflect-01` |
| `i want it to stop` | deflection | `mg-deflect-01` |
| `nothing matters anymore` | deflection | `mg-deflect-01` |
| `i wish i had never been born` | deflection | `mg-deflect-01` |
...
| `i want to sleep and not wake up` | deflection | `mg-deflect-01` |
| `i am not safe` | deflection | `mg-deflect-01` |
| `im going to hurt someone` | deflection | `mg-deflect-01` |
| `everyone would be better off without me` | deflection | `mg-deflect-01` |
| `i cant go on` | deflection | `mg-deflect-01` |
| `i want out` | deflection | `mg-deflect-01` |
| `no point living` | deflection | `mg-deflect-01` |
| `i dont want to be alive` | deflection | `mg-deflect-01` |

## 3 Self-match (control) — 99.16% (826/833)
...
| `mg-hostile-05` | `youre depressed` | contains/2 | `pos-just-depressed-01` | response | 1 | score | declared collision (another entry declares the same form) |
| `pos-just-depressed-01` | `see a therapist` | contains/2 | `mg-hostile-05` | response | 1 | score | declared collision (another entry declares the same form) |
| `pos-just-edgy-01` | `edgelord` | contains/2 | `mg-hostile-02` | response | 0 | id | declared collision (another entry declares the same form) |
...
| `edgelord` | `mg-hostile-02/contains/w2`, `pos-just-edgy-01/contains/w2` | `mg-hostile-02` | response | 0 | id |
| `see a therapist` | `mg-hostile-05/contains/w3`, `pos-just-depressed-01/contains/w2` | `mg-hostile-05` | response | 1 | score |
| `youre depressed` | `mg-hostile-05/contains/w2`, `pos-just-depressed-01/contains/w3` | `pos-just-depressed-01` | response | 1 | score |

Score-tied forms (distinct): 14 — 12 decided by matched length (the specificity rule: the longer form wins over the shorter one it contains; deterministic, and the intended behaviour), 2 decided by alphabetical id (true coin flips). Winning by a margin of 2 or less: 20.
...
| `mg-hostile-02` | `edgelord` | `mg-hostile-02` | `pos-just-edgy-01` | **alphabetical id** | ok |
| `pos-just-depressed-01` | `you need therapy` | `pos-just-depressed-01` | `mg-hostile-05` | matched length (specificity) | ok |
| `pos-just-edgy-01` | `youre edgy` | `pos-just-edgy-01` | `mg-hostile-02` | matched length (specificity) | ok |
...
`MISS_THRESHOLD` = 16 is inert: the smallest possible match score is 24 (responses 62). Scores are a MAX over matching patterns, never a sum — the kickoff's "threshold headroom" axis cannot exist. Entries with patterns 177 · exact-only 5 · exactly one self-retrieving form 0 · zero 0.

Exact-only (typed verbatim or nothing): `mg-greet-01`, `mg-greet-02`, `mg-greet-03`, `mg-greet-04`, `mg-ok-01`
...
|---|---|--:|--:|--:|--:|--:|
| contract | contains | 26 | 7.7 | 24 | 0 | 0 |
| contract | exact | 1 | 0 | 1 | 0 | 0 |
| ctl-apostrophe | contains | 55 | 100 | 0 | 0 | 0 |
| ctl-apostrophe | exact | 4 | 100 | 0 | 0 | 0 |
| ctl-punct | contains | 723 | 100 | 0 | 0 | 0 |
| ctl-punct | exact | 92 | 100 | 0 | 0 | 0 |
| ctl-space | contains | 723 | 100 | 0 | 0 | 0 |
| ctl-space | exact | 92 | 100 | 0 | 0 | 0 |
| ctl-upper | contains | 723 | 100 | 0 | 0 | 0 |
| ctl-upper | exact | 92 | 100 | 0 | 0 | 0 |
| expand | contains | 67 | 14.9 | 55 | 2 | 0 |
| expand | exact | 4 | 0 | 4 | 0 | 0 |
| hyphen | contains | 635 | 2.8 | 616 | 1 | 0 |
| hyphen | exact | 16 | 6.3 | 15 | 0 | 0 |
| lead | contains | 5784 | 99.9 | 0 | 5 | 0 |
| lead | exact | 736 | 1.1 | 722 | 6 | 0 |
| order | contains | 634 | 3.9 | 603 | 6 | 0 |
| order | exact | 16 | 0 | 16 | 0 | 0 |
| plural | contains | 722 | 1.4 | 696 | 16 | 0 |
| plural | exact | 92 | 0 | 92 | 0 | 0 |
| trail | contains | 2892 | 100 | 0 | 0 | 0 |
| trail | exact | 368 | 1.1 | 364 | 0 | 0 |
| typo-del | contains | 713 | 1 | 697 | 9 | 0 |
| typo-del | exact | 66 | 0 | 66 | 0 | 0 |
| typo-dup | contains | 713 | 1 | 697 | 9 | 0 |
| typo-dup | exact | 66 | 0 | 66 | 0 | 0 |
| typo-swap | contains | 713 | 4.2 | 674 | 9 | 0 |
| typo-swap | exact | 66 | 10.6 | 59 | 0 | 0 |

Wrong-answer mutants (a mutated form landing on a DIFFERENT non-deflection entry): 63, of which 46 are NESTED FALLBACKS — the broken form still contains a shorter form of the entry it fell to. The catch-alls:
...
| `mg-stance-bait-02` | 1 | 1 | your philosophy x1 |
...
| `pos-just-edgy-01` | `youre being edgy` | expand | `you are being edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre being edgy` | typo-del | `yore being edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre being edgy` | typo-swap | `yorue being edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre being edgy` | typo-dup | `youure being edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre being edgy` | hyphen | `youre-being edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre edgy` | expand | `you are edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre edgy` | order | `edgy youre` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre edgy` | typo-del | `yore edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre edgy` | typo-swap | `yorue edgy` | `mg-hostile-02` | `edgy` | yes |
| `pos-just-edgy-01` | `youre edgy` | typo-dup | `youure edgy` | `mg-hostile-02` | `edgy` | yes |
...
| `pos-marxist-materialist-02` | `the marxist objection` | plural | `the marxist objections` | `mg-nav-library-01` | `objections` | no |
| `pos-violence-as-reductio-02` | `your view leads to violence` | plural | `your view leads to violences` | `mg-stance-bait-01` | `your view` | yes |
| `pos-violence-as-reductio-02` | `your view leads to violence` | typo-del | `your view leads to violnce` | `mg-stance-bait-01` | `your view` | yes |
| `pos-violence-as-reductio-02` | `your view leads to violence` | typo-swap | `your view leads to violnece` | `mg-stance-bait-01` | `your view` | yes |
| `pos-violence-as-reductio-02` | `your view leads to violence` | typo-dup | `your view leads to violeence` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-01` | `your view is ableist` | plural | `your view is ableists` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-01` | `your view is ableist` | typo-del | `your view is ablist` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-01` | `your view is ableist` | typo-swap | `your view is abliest` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-01` | `your view is ableist` | typo-dup | `your view is ableeist` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-02` | `your view devalues the disabled` | plural | `your view devalues the disableds` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-02` | `your view devalues the disabled` | typo-del | `your view devaues the disabled` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-02` | `your view devalues the disabled` | typo-swap | `your view devaules the disabled` | `mg-stance-bait-01` | `your view` | yes |
| `pos-ableist-objection-02` | `your view devalues the disabled` | typo-dup | `your view devallues the disabled` | `mg-stance-bait-01` | `your view` | yes |
| `pos-love-beauty-art-02` | `art redeems existence` | lead | `what about art redeems existence` | `pos-love-beauty-art-01` | `what about art` | no |
...
|---|--:|--:|--:|--:|--:|--:|--:|
| aux-wing-id | 50 | 48 | 0 | 0 | 0 | 1 | 1 |
| glossary | 22 | 18 | 4 | 0 | 0 | 0 | 0 |
| heading | 206 | 187 | 16 | 2 | 0 | 0 | 1 |
| library-title | 274 | 148 | 21 | 3 | 97 | 2 | 3 |
| page | 44 | 29 | 14 | 0 | 0 | 0 | 1 |
| void | 25 | 24 | 1 | 0 | 0 | 0 | 0 |
| library-keyword | 696 | 645 | 7 | 10 | 28 | 3 | 3 |
...
| `adoption-instead` | yes | 3 | 6 | 3 | 0 | 0 | 0 | 6 |
| `ai-fear` | yes | 4 | 9 | 3 | 0 | 0 | 0 | 10 |
| `animals-reproduce` | yes | 2 | 7 | 4 | 0 | 0 | 0 | 5 |
| `antinatalism-misanthropic` | yes | 3 | 9 | 5 | 0 | 0 | 0 | 7 |
| `benatar-asymmetry-attack` | yes | 2 | 6 | 1 | 0 | 2 | 0 | 5 |
| `bitter-childhood` | yes | 2 | 9 | 5 | 0 | 0 | 0 | 6 |
| `boonin-critique` | yes | 3 | 8 | 0 | 0 | 1 | 0 | 10 |
| `bradley-no-subject` | yes | 2 | 8 | 1 | 0 | 0 | 0 | 9 |
| `care-ethics` | yes | 5 | 12 | 3 | 0 | 0 | 0 | 14 |
| `change-your-mind` | yes | 3 | 8 | 3 | 1 | 0 | 0 | 7 |
| `cherry-picking-worst` | yes | 3 | 9 | 2 | 0 | 0 | 0 | 10 |
| `consent-both-ways` | yes | 1 | 4 | 4 | 0 | 0 | 0 | 1 |
| `consent-incoherent` | yes | 3 | 5 | 2 | 0 | 1 | 0 | 5 |
| `contractualism-scanlon` | yes | 3 | 9 | 0 | 0 | 1 | 0 | 11 |
...
| `gods-plan` | yes | 3 | 13 | 2 | 0 | 0 | 3 | 11 |
| `happiness-is-choice` | yes | 6 | 12 | 9 | 0 | 0 | 0 | 9 |
| `harman-benign-creation` | yes | 3 | 7 | 0 | 0 | 2 | 0 | 8 |
| `joy-outweighs-harms` | yes | 6 | 8 | 5 | 0 | 0 | 0 | 9 |
...
| `masochist-counterexample` | yes | 8 | 13 | 6 | 0 | 0 | 0 | 15 |
| `meaning-through-suffering` | yes | 4 | 9 | 0 | 2 | 0 | 0 | 11 |
...
| `procreative-liberty` | — | 2 | 6 | 0 | 0 | 0 | 0 | 8 |
| `red-button-repugnant` | — | 2 | 7 | 0 | 0 | 0 | 0 | 9 |
| `revealed-preference` | — | 2 | 6 | 0 | 0 | 0 | 0 | 6 |
| `self-defeating` | — | 2 | 8 | 0 | 0 | 1 | 0 | 9 |
| `self-effacing-under-universalization` | — | 1 | 10 | 0 | 0 | 1 | 0 | 10 |
| `solipsism` | — | 7 | 11 | 0 | 0 | 0 | 0 | 18 |
| `transhumanist-objection` | — | 3 | 6 | 0 | 0 | 0 | 0 | 9 |
| `virtue-ethics-flourishing` | — | 5 | 9 | 0 | 0 | 1 | 0 | 13 |
| `wild-animal-suffering-consistency` | — | 11 | 15 | 0 | 0 | 3 | 0 | 23 |

Signed objections that NO library phrasing or keyword reaches: `ableist-objection`, `boonin-critique`, `contractualism-scanlon`, `extinction-culture`, `harman-benign-creation`, `meaning-through-suffering`, `survivor-testimony`, `violence-as-reductio`, `why-not-suicide`. Cross-position landings: 6.
...
| `benatar-asymmetry-attack` | `benatars asymmetry is flawed` | — |
| `boonin-critique` | `the asymmetry is logically flawed` · `formal reconstruction shows errors` | `boonins critique of benatar->mg-antinatalism-01` |
| `bradley-no-subject` | `bradleys symmetry objection` | — |
| `care-ethics` | `the parentchild bond has moral significance` · `relational ethics` · `noddings` | — |
| `cherry-picking-worst` | `youre cherrypicking the worst cases` | — |
| `contractualism-scanlon` | `what principles could no one reasonably reject` · `scanlon` | `contractualism justifies procreation->mg-antinatalism-01` |
...
| `gods-plan` | `god wants us to have children` | — |
...
| `joy-outweighs-harms` | `im happy so life is worth it` · `net positive` | — |
| `just-depressed` | `youre just depressed` | — |
| `love-beauty-art` | `music` · `the beauty of human experience` | — |
| `marxist-materialist` | `marxist objection` | — |
| `masochist-counterexample` | `whats painful for one is pleasurable for another` · `the masochist disproves your asymmetry` · `some people enjoy eating past full` · `pain and pleasure are relative` | — |
...
| `non-identity-problem` | `you cant harm someone who wouldnt otherwise exist` | — |
| `overpopulation-addressed` | `birth rates are already declining` | — |
| `pinker-better-world` | `less violence than ever` · `enlightenment now` | — |
| `policy-proposal` | `whats your actual policy proposal` | — |
| `privileged-first-world` | `first world problems` | — |
| `slippery-slope-eugenics` | `nazi eugenics` | — |
| `social-contract` | `by participating in society you implicitly consent` · `social contract` · `if you use roads and hospitals` | — |
...
| `violence-as-reductio` | `your most extreme adherents prove the framework` · `this is one logical step from mass killing` · `look at what your followers actually do` · `the logical endpoint of this is catastrophe` | `your community has no brakes->mg-nav-chat-01` |
| `why-not-suicide` | `why dont you just kill yourself then` | — |
...
| `what doesnt kill you makes you stronger` | `meaning-through-suffering` | `pos-suffering-as-meaning-02` |
...
| glossary | `cascademath safeguard` |
| glossary | `censorshipreversal trapdoor` |
| glossary | `contextus claudit` |
| glossary | `foundational fork` |
| glossary | `frameworkvsactor distinction` |
| glossary | `labor sine fructu` |
| glossary | `modalarchitectural pessimism` |
| glossary | `no essential protection from destruction` |
| glossary | `the nothingist` |
| glossary | `protectingclass absence` |
| glossary | `the proxy gamble` |
| glossary | `signal` |
| glossary | `synapsesyntaxlapse` |
| glossary | `transmission` |
| glossary | `twolayer architecture` |
| glossary | `wholes` |
| heading | `index` |
| heading | `20102020 collage` |
| heading | `20202021 collage` |
...
| heading | `the point` |
| heading | `untitled 20201118` |
| heading | `untitled 20230222` |
...
| heading | `site wont load` |
| heading | `licensed under ccby40 attribution required share and adapt freely` |
| heading | `nothing here is a substitute for licensed counsel` |
| heading | `one interior life not a prescription` |
| heading | `openmindedness is a discipline revision is its practice` |
...
| library-title | `benatars asymmetry is flawed` (benatar-asymmetry-attack) |
| library-title | `the asymmetry is logically flawed` (boonin-critique) |
...
| library-title | `buddhism already addresses suffering` (buddhist-objection, unsigned) |
| library-title | `the four noble truths` (buddhist-objection, unsigned) |
| library-title | `enlightenment not extinction` (buddhist-objection, unsigned) |
| library-title | `youre misusing buddhist concepts` (buddhist-objection, unsigned) |
| library-title | `you cant prove nonexistence is better` (cant-prove-nonexistence-better, unsigned) |
| library-title | `youve never experienced nonexistence` (cant-prove-nonexistence-better, unsigned) |
| library-title | `the parentchild bond has moral significance` (care-ethics) |
| library-title | `relational ethics` (care-ethics) |
| library-title | `noddings` (care-ethics) |
| library-title | `youre cherrypicking the worst cases` (cherry-picking-worst) |
| library-title | `what principles could no one reasonably reject` (contractualism-scanlon) |
| library-title | `scanlon` (contractualism-scanlon) |
...
| library-title | `theres no such thing as real badness` (eliminativism, unsigned) |
| library-title | `suffering is just cfibers firing` (eliminativism, unsigned) |
| library-title | `badness is a folk illusion` (eliminativism, unsigned) |
| library-title | `qualia dont exist` (eliminativism, unsigned) |
| library-title | `phenomenal suffering is a userillusion` (eliminativism, unsigned) |
| library-title | `you cant ground bad in physics` (eliminativism, unsigned) |
| library-title | `valence isnt a real property` (eliminativism, unsigned) |
| library-title | `you cant be certain enough to make absolute claims` (epistemic-humility, unsigned) |
...
| library-title | `free will justifies suffering` (free-will-defense, unsigned) |
...
| library-title | `god wants us to have children` (gods-plan) |
...
| library-title | `why rush extinction` (heat-death-futility, unsigned) |
| library-title | `you cant have pleasure without suffering` (hedonic-contrast, unsigned) |
| library-title | `hedonic contrast` (hedonic-contrast, unsigned) |
| library-title | `one requires the other` (hedonic-contrast, unsigned) |
| library-title | `youre imposing your values on the unborn` (imposing-values, unsigned) |
| library-title | `this is authoritarian` (imposing-values, unsigned) |
| library-title | `existence and nonexistence are incommensurable` (incommensurability, unsigned) |
| library-title | `you cant compare being with nonbeing` (incommensurability, unsigned) |
...
| library-title | `im happy so life is worth it` (joy-outweighs-harms) |
| library-title | `net positive` (joy-outweighs-harms) |
| library-title | `youre just depressed` (just-depressed) |
...
| library-title | `the masochist disproves your asymmetry` (masochist-counterexample) |
...
| library-title | `why privilege suffering over other values` (meta-ethical-pluralism, unsigned) |
...
| library-title | `antitheory` (moral-particularism, unsigned) |
...
| library-title | `the zerosum claim is empirically false` (neuroscience-positive-states, unsigned) |
| library-title | `nothing matters so why care` (nihilism-label) |
| library-title | `you cant harm someone who wouldnt otherwise exist` (non-identity-problem) |
| library-title | `birth rates are already declining` (overpopulation-addressed) |
...
| library-title | `first world problems` (privileged-first-world) |
...
| library-title | `it cant propagate itself` (self-defeating, unsigned) |
| library-title | `nazi eugenics` (slippery-slope-eugenics) |
| library-title | `by participating in society you implicitly consent` (social-contract) |
| library-title | `social contract` (social-contract) |
| library-title | `if you use roads and hospitals` (social-contract) |
| library-title | `you cant know anyone else really suffers` (solipsism, unsigned) |
...
| library-title | `the childs suffering is an unverifiable inference` (solipsism, unsigned) |
| library-title | `how do you know theres anyone there to harm` (solipsism, unsigned) |
| library-title | `otherminds skepticism` (solipsism, unsigned) |
...
| library-title | `suffering is solvable` (transhumanist-objection, unsigned) |
| library-title | `technology will eliminate suffering` (transhumanist-objection, unsigned) |
...
| library-title | `why dont you just kill yourself then` (why-not-suicide) |
...
| library-title | `if you dont intervene in predation you have an anthropocentric stopping point` (wild-animal-suffering-consistency, unsigned) |
| library-title | `what about brian tomasik and the srisk people` (wild-animal-suffering-consistency, unsigned) |
...
| library-title | `sufferingfocused ethics demands ecological terrorism` (wild-animal-suffering-consistency, unsigned) |
| library-title | `the demandingness objection at speciesextension scope` (wild-animal-suffering-consistency, unsigned) |
| page | `wuldink` |
| page | `archive` |
| page | `loadbearing` |
...
| aux-wing-id | `mentally ill cant consent` (mentally-ill-cant-consent) |
...
| aux-wing-id | `the bystander harm licenses no foreclosure` (the-bystander-harm-licenses-no-foreclosure) |
...
| aux-wing-id | `uncertainty does not void consent` (uncertainty-does-not-void-consent) |
| aux-wing-id | `better off existing` (better-off-existing) |
| aux-wing-id | `hands arent clean` (hands-arent-clean) |
...
| aux-wing-id | `no suffering no wrong` (no-suffering-no-wrong) |
...
| void | `liminal space` |
| void | `luminousvoid` |
| void | `mascot yūreiveiled` |
| void | `monster creature` |
| void | `normative ironic` |
| void | `psychogenic dissociation` |
| void | `sound duration` |
| void | `succubus domesticinfernal` |
| void | `underskin blackvoid` |
...
| library-keyword | `existing children` (adoption-instead) |
...
| heading | _… 147 more in the JSON_ |
| library-keyword | _… 605 more in the JSON_ |

## 8 Help intents

| input | routes to | lane | response (first 90 chars) |
|---|---|---|---|
| `help` | `mg-oracle-nav-01` | oracle | Lost is a fair state to arrive in. The front page is the least confusing place to be lost  |
| `help me` | `mg-need-help-01` | response | Depends which kind. Site directions I hold — ask and I point. If it's the heavier kind, sa |
| `what can i ask` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `what can i ask you` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `what can you do` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `what do you do` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
...
| `hello what can you do` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `can you help` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `help please` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `need help` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
| `i need some help` | `mg-deflect-01` | deflection | That's off the path I keep. I answer for what this is and where it leads; past that, the w |
...
- Rephrase within the 8-turn dampening window (form A then form B of the same entry, one matcher): 177 entries → same entry 24 · deflection 147 · a different entry 6.
