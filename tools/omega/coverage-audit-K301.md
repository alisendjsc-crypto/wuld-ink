...
corpus `src/components/omega-corpus-mrgrey.json` · engine `src/components/yurei-oracle.js` · entries 188 · patterns 888 · fresh Matcher per probe · measure-only

## Findings (numbers; the reading is in the K295 stratum)

- Crisis floor: fires by construction at stage 1 (any hit, score>0, no threshold, before every other lane); bare 37/37, embedded 180/180, repeated ×4 and post-conversation all crisis. The floor's own coverage of adjacent distress phrasings: 10/24 (§2).
- Self-match (control): 98.99% — every failure is a declared collision or the floor arbitrating with itself; undeclared steals: 0 (§3).
- Ambiguity: 8 declared collisions; 1 true coin flips (score-tied, decided by alphabetical id: `hi`); 12 score ties decided by matched length, i.e. the specificity rule working (§4).
...
- Brittleness: `contains` forms survive a leading filler 99.9% and a trailing one 99.9%; `exact` forms survive a leading filler 1.1%; contraction expansion survives 100%; word order 4%; one-edit typos ≈0.9% (§6).
- Subject coverage: 82 library objections, 53 signed; 9 signed objections that no library phrasing or keyword reaches; 29 signed objections with deflecting title phrasings (candidate triggers); 6 cross-position landings (§7).
- Help: 18/32 help-shaped inputs deflect (§8).
- Dynamics: a rephrase of the same response entry within the 8-turn dampening window deflects for 136/181 entries (§9).
...
- bare forms -> crisis: 37/37 · embedded/stacked -> crisis: 180/180 · repeated x4 on one matcher: crisis,crisis,crisis,crisis · after an oracle + a position hit: crisis
...
## 3 Self-match (control) — 98.99% (879/888)
...
Score-tied forms (distinct): 13 — 12 decided by matched length (the specificity rule: the longer form wins over the shorter one it contains; deterministic, and the intended behaviour), 1 decided by alphabetical id (true coin flips). Winning by a margin of 2 or less: 61.
...
`MISS_THRESHOLD` = 16 is inert: the smallest possible match score is 24 (responses 61). Scores are a MAX over matching patterns, never a sum — the kickoff's "threshold headroom" axis cannot exist. Entries with patterns 181 · exact-only 5 · exactly one self-retrieving form 0 · zero 0.
...
|---|---|--:|--:|--:|--:|--:|
| contract | contains | 84 | 92.9 | 6 | 0 | 0 |
| contract | exact | 5 | 80 | 1 | 0 | 0 |
| ctl-apostrophe | contains | 12 | 100 | 0 | 0 | 0 |
| ctl-punct | contains | 758 | 100 | 0 | 0 | 0 |
| ctl-punct | exact | 93 | 100 | 0 | 0 | 0 |
| ctl-space | contains | 758 | 100 | 0 | 0 | 0 |
| ctl-space | exact | 93 | 100 | 0 | 0 | 0 |
| ctl-upper | contains | 758 | 100 | 0 | 0 | 0 |
| ctl-upper | exact | 93 | 100 | 0 | 0 | 0 |
| expand | contains | 12 | 100 | 0 | 0 | 0 |
| hyphen | contains | 655 | 100 | 0 | 0 | 0 |
| hyphen | exact | 16 | 100 | 0 | 0 | 0 |
| lead | contains | 6064 | 99.9 | 0 | 5 | 0 |
| lead | exact | 744 | 1.1 | 729 | 7 | 0 |
| order | contains | 654 | 4 | 577 | 51 | 0 |
| order | exact | 16 | 0 | 16 | 0 | 0 |
| plural | contains | 756 | 1.6 | 699 | 45 | 0 |
| plural | exact | 93 | 0 | 93 | 0 | 0 |
| trail | contains | 3032 | 99.9 | 0 | 2 | 0 |
| trail | exact | 372 | 1.1 | 367 | 1 | 0 |
| typo-del | contains | 748 | 0.9 | 718 | 23 | 0 |
| typo-del | exact | 67 | 0 | 67 | 0 | 0 |
| typo-dup | contains | 748 | 0.9 | 718 | 23 | 0 |
| typo-dup | exact | 67 | 0 | 67 | 0 | 0 |
| typo-swap | contains | 748 | 4.4 | 692 | 23 | 0 |
| typo-swap | exact | 67 | 10.4 | 60 | 0 | 0 |

Wrong-answer mutants (a mutated form landing on a DIFFERENT non-deflection entry): 180, of which 156 are NESTED FALLBACKS — the broken form still contains a shorter form of the entry it fell to. The catch-alls:

| landing entry | wrong answers | nested fallbacks | via form |
|---|--:|--:|---|
| `mg-topical-deflect-01` | 116 | 115 | consent x50, suffering x31, asymmetry x15, harm x6, birth x5, children x4, nihilism x4, nonexistence x1 |
...
| `pos-marxist-materialist-01` | `suffering is caused by capitalism` | order | `is suffering caused by capitalism` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-01` | `suffering is caused by capitalism` | typo-del | `suffering is caused by capitlism` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-01` | `suffering is caused by capitalism` | typo-swap | `suffering is caused by capitlaism` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-01` | `suffering is caused by capitalism` | typo-dup | `suffering is caused by capitaalism` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-02` | `abolish capitalism and suffering ends` | plural | `abolish capitalism and suffering end` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-02` | `abolish capitalism and suffering ends` | order | `capitalism abolish and suffering ends` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-02` | `abolish capitalism and suffering ends` | typo-del | `abolish capitlism and suffering ends` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-02` | `abolish capitalism and suffering ends` | typo-swap | `abolish capitlaism and suffering ends` | `mg-topical-deflect-01` | `suffering` | yes |
| `pos-marxist-materialist-02` | `abolish capitalism and suffering ends` | typo-dup | `abolish capitaalism and suffering ends` | `mg-topical-deflect-01` | `suffering` | yes |
...
| library-title | 274 | 129 | 19 | 25 | 96 | 2 | 3 |
| page | 44 | 29 | 14 | 0 | 0 | 0 | 1 |
| void | 25 | 24 | 1 | 0 | 0 | 0 | 0 |
| library-keyword | 696 | 626 | 7 | 30 | 27 | 3 | 3 |
...
| `boonin-critique` | yes | 3 | 8 | 0 | 0 | 1 | 2 | 8 |
...
| `non-identity-problem` | yes | 3 | 8 | 2 | 0 | 0 | 1 | 8 |
...
| `non-identity-problem` | `the non identity problem` | — |
...
| library-keyword | _… 586 more in the JSON_ |
...
- Rephrase within the 8-turn dampening window (form A then form B of the same entry, one matcher): 181 entries → same entry 28 · deflection 136 · a different entry 17.
